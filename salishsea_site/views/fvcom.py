...
second_narrows_current_figure = FigureMetadata(
    title="Near-Surface Current at 2nd Narrows", svg_name="2ndNarrows_current"
)
...
def results_index(request):
    ...
    grid_rows = (
        ...
        ("x2 nowcast water levels", "nowcast-x2", tide_stn_water_level_figure_group),
        ("r12 nowcast water levels", "nowcast-r12", tide_stn_water_level_figure_group),
        ("x2 forecast water levels", "forecast-x2", tide_stn_water_level_figure_group),
    )
    grid_dates = {
...
def _exclude_missing_dates(dates, figures, run_type):
    return (
        (
            d
            if any(fig.available(run_type, d, model="fvcom") for fig in figures)
            else None
        )
        for d in dates
    )
...
@view_config(
    route_name="fvcom.results.nowcast-x2.publish", renderer="fvcom/publish.mako"
)
# Legacy route
...
@view_config(
    route_name="fvcom.results.forecast-x2.publish", renderer="fvcom/publish.mako"
)
# Legacy route
...
def _values_for_publish_template(request, run_type):
    ...
    available_figures = {
        ...
        "2nd narrows currents": [
            second_narrows_current_figure.available(run_type, results_date, "fvcom")
        ],
        "image loops": [],
    }
    for image_loop in image_loops.loops:
        images_available = image_loop.available(run_type, results_date, "fvcom")
        if images_available:
            ...
            image_loop.hrs = image_loop.hours(
                ...
                file_dates=arrow.Arrow.range(
                    "day", results_date, results_date.shift(hours=+run_duration)
                ),
            )
